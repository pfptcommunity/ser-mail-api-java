...
/**
 * A client for making HTTP requests with OAuth 2.0 authentication. This class manages token
 * retrieval and refresh operations, ensuring that requests are sent with a valid access token. It
 * supports asynchronous requests and handles token expiration gracefully using thread-safe
 * mechanisms.
 */
public class OAuthHttpClient {
  ...
  /**
   * Ensures that a valid token is available before making a request. Checks the current token's
   * validity and refreshes it if necessary, using a lock to prevent concurrent refreshes.
   *
  ...
  /**
   * Refreshes the OAuth token synchronously. Sends a request to the token endpoint to obtain a new
   * access token and updates the stored token and expiration time.
   *
   * @throws HttpTokenRefreshException if the refresh operation fails due to network issues, invalid
    ...
    try {
      ...
      try (JsonReader reader = Json.createReader(new StringReader(response.body()))) {
        ...
        if (newAccessToken == null) {
          throw new HttpTokenRefreshException(
        ...
        if (json.containsKey("token_expires_date_time")) {
          newExpiration =
              Instant.parse(json.getString("token_expires_date_time"))
                  .minusSeconds(tokenRefreshOffset);
        } else if (json.containsKey("expires_in")) {
          newExpiration = Instant.now().plusSeconds(json.getInt("expires_in") - tokenRefreshOffset);
        } else {
          throw new HttpTokenRefreshException(
              "OAuth token response is missing expiration details.");
        }
  ...
  /**
   * Sends an HTTP request asynchronously and returns a CompletableFuture with the response. This
   * method handles OAuth authentication and delegates the request to an underlying HTTP client.
   *
  ...
  public <T> CompletableFuture<HttpResponse<T>> sendAsync(
    ...
    try {
      ...
      failedFuture.completeExceptionally(
          new HttpRequestException("Failed to refresh token before sending request", e));
      return failedFuture;
    }
    ...
    return httpClient
        .sendAsync(authorizedRequestBuilder.build(), responseBodyHandler)
        .handle(
            (response, throwable) -> {
              if (throwable != null) {
                String errorMessage =
                    String.format(
                        "Asynchronous request failed: %s %s", request.method(), request.uri());
  ...
  /**
   * Creates a new HttpRequest.Builder by copying an existing HttpRequest. Preserves the original
   * request's properties except for the Authorization header, which is managed separately.
   *
